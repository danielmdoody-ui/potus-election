...
function initENCongressBoard(){
  ...
  if(window._SIM){
    ...
    const rawBonus = natLead * 0.12 + momBonus;
...
function finishElectionNight(){
  ...
  if(_enCongress){
    ...
    // Compute deep-state coattail bonus affecting congress results
    const _deepCoattailBonus = (()=>{
      ...
      // For each deep safe state player won with big margin, add fractional coattail
      let senatBonus = 0, houseBonus = 0;
      GS.states.forEach(s => {
        const isSafePlayer = playerIsDem ? s.lean > 18 : s.lean < -18;
        if(isSafePlayer && s.genLead > 10) {
          // Strong performance in safe states provides coattail effect
          const coattail = Math.min((s.genLead - 10) / 60, 0.04); // up to 4% per state
          senatBonus += coattail * 0.15; // modest senate boost
          houseBonus += coattail * 2.5;  // more meaningful house boost
        }
      });
      return { senate: Math.round(senatBonus), house: Math.round(houseBonus) };
    })();
    GS._congressResult = {
      senate: {
