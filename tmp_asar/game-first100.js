...
function _applyEvaluationResult(result,originalResponse){
  ...
  if(currentPhaseId==='legislation'&&(goodGrade||result.grade==='C'))
...
function _checkMilestone(){
  const checks=[{key:'cabinet',day:8},{key:'agenda',day:14},{key:'legislation',day:42},{key:'crisis_peak',day:72},{key:'address',day:99}];
...
function _pickFallbackSituation(phaseId){
  ...
  if(!av.length){P100._usedByPhase[phaseId]=[];av=pool;}
...
function injectBreakdownButton(won){
  if(!won||document.getElementById('bk-presidency-btn'))return;
