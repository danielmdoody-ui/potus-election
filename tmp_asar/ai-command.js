...
(function () {
  ...
  function localAdvice(actionId, stateCode, text) {
    ...
    if ((actionId === 'visit' || actionId === 'grass' || actionId === 'townhall' || actionId === 'canvass') && sn) {
      ...
      if (lead == null) return `${actionName} in ${sn} — polling's murky here, but ${ev} EV is worth the trip.${homeBonus}`;
      if (lead > 5) return pick([
        `${sn} is already ${lead.toFixed(0)} points yours — ${ev} EV in the bag. A ${actionName.toLowerCase()} here shores up the base, but you might be leaving closer fights unattended.`,
        `You're up ${lead.toFixed(0)} in ${sn}. Solid state, but don't waste too many weeks on wins you already own — ${ev} EV isn't worth neglecting real battlegrounds.`,
      ]);
      if (lead < -5) return pick([
        `You're down ${Math.abs(lead).toFixed(0)} points in ${sn} (${ev} EV). This is a fight, not a formality — a ${actionName.toLowerCase()} here could shift momentum, but only if you can close the gap in time.`,
        `${sn} is hostile territory — trailing by ${Math.abs(lead).toFixed(0)} with ${ev} EV on the line. Worth the swing if you've got no softer targets.`,
      ]);
      return pick([
        `${sn} is a knife fight — ${lead > 0 ? `you lead by just ${lead.toFixed(0)}` : lead < 0 ? `trailing by ${Math.abs(lead).toFixed(0)}` : 'dead even'} with ${ev} EV at stake.${homeBonus} Best possible use of a week.`,
        `${ev} EV and essentially a coin flip in ${sn}. A ${actionName.toLowerCase()} here is textbook battleground strategy — get in front of those voters.${homeBonus}`,
        `This is exactly the kind of state that wins elections. ${sn}'s margin is razor-thin and those ${ev} electoral votes aren't going to win themselves.`,
      ]);
    }
    ...
    return top
      ? `Your most contested state right now is ${top.name} (${top.ev} EV) - a rally or ground game push there has the highest payoff this week.`
      ...
      if (action) {
        ...
        if (stateCode && stateName) {
          ...
          if (funds < 6) {
            ...
            chips.push({ label: '💰 Fundraise', actionId: 'fundraise', state: '' });
          }
