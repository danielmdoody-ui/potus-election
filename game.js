...
function setupUI(){
  ...
  if(sel) sel.onchange=()=>{
    ...
    const selectedActionObj = ACTIONS.find(x=>x.id===GS.selectedAction) || UNDERDOG_ACTIONS.find(x=>x.id===GS.selectedAction);
    if(GS.targetState && selectedActionObj?.needsState && MINIGAME_ACTIONS.has(GS.selectedAction)){
...
// Actions with minigames — local AI mode can run a minigame flow for every action.
// Build this from live action definitions so coverage stays complete as actions change.
const MINIGAME_ACTIONS = new Set([
  ...ACTIONS.map(a=>a.id),
  ...UNDERDOG_ACTIONS.map(a=>a.id),
  'town_hall' // alias used by older command parsing paths
]);
...
function selectAction(id){
  ...
  if(MINIGAME_ACTIONS.has(id)){
    ...
    // State-targeted actions need a state selected first — minigame fires after state pick
    if(a.needsState){
...
// Ensure local-AI minigames are distinct for every action id, including newer actions.
const AI_MG_ACTION_HINTS = {
  visit: { scene:'airport hangar rally', challenge:'excite turnout without overpromising', audience:'swing-state crowd' },
  fundraise: { scene:'private donor dinner', challenge:'ask directly for money', audience:'major donors' },
  speech: { scene:'policy rollout speech', challenge:'make complex policy sound clear', audience:'national TV audience' },
  attack: { scene:'war room attack-ad meeting', challenge:'go hard without seeming cruel', audience:'independent voters' },
  coalition: { scene:'cross-bloc coalition summit', challenge:'unite clashing priorities', audience:'bloc leaders' },
  debate: { scene:'debate prep room', challenge:'land sharp answers under pressure', audience:'debate viewers' },
  grass: { scene:'field office volunteer kickoff', challenge:'motivate organizers in difficult terrain', audience:'local volunteers' },
  adblitz: { scene:'ad strategy war room', challenge:'pick a message that cuts through noise', audience:'persuadable voters' },
  townhall: { scene:'live town hall Q&A', challenge:'answer tough questions with empathy', audience:'undecided families' },
  town_hall: { scene:'live town hall Q&A', challenge:'answer tough questions with empathy', audience:'undecided families' },
  surrogate: { scene:'surrogate coordination call', challenge:'keep allies on one message', audience:'regional media markets' },
  crisis: { scene:'crisis response room', challenge:'stabilize trust quickly', audience:'national press corps' },
  press_tour: { scene:'press gaggle gauntlet', challenge:'avoid headline traps', audience:'hostile reporters' },
  fundraise_calls: { scene:'high-stakes donor call block', challenge:'tailor the ask in seconds', audience:'bundlers and repeat donors' },
  union_speech: { scene:'union hall address', challenge:'prove you back workers with specifics', audience:'union households' },
  climate_push: { scene:'climate policy forum', challenge:'balance urgency with affordability', audience:'young and suburban voters' },
  crime_ad: { scene:'public safety ad shoot', challenge:'sound strong without sounding extreme', audience:'suburban persuadables' },
  econ_msg: { scene:'economic message briefing', challenge:'sell pocketbook relief credibly', audience:'working-class independents' },
  college_push: { scene:'campus coalition event', challenge:'mobilize turnout, not just applause', audience:'students and college grads' },
  rural_tour: { scene:'rural diner stop', challenge:'connect without sounding scripted', audience:'small-town voters' },
  canvass: { scene:'door-knock launch', challenge:'adapt pitch house by house', audience:'target precinct voters' },
  spin_room: { scene:'post-debate spin room', challenge:'rewrite the narrative before midnight', audience:'party insiders and press' },
  oppo_rapid: { scene:'rapid-response desk', challenge:'counterattack with facts instantly', audience:'social media audience' },
  delegate_push: { scene:'delegate whip operation', challenge:'secure commitments quietly', audience:'uncommitted delegates' },
  national_address: { scene:'prime-time national address', challenge:'go big without sounding desperate', audience:'entire electorate' },
  attack_blitz: { scene:'negative-ad blitz decision', challenge:'damage opponent while limiting backlash', audience:'low-information voters' },
  vp_announce: { scene:'surprise VP rollout', challenge:'sell the pick before critics define it', audience:'swing-state media' },
  policy_pivot: { scene:'major platform pivot briefing', challenge:'shift position without looking fake', audience:'base and independents' },
  roast: { scene:'comedy roast spotlight', challenge:'be sharp, not mean', audience:'viral online audience' },
  strategic_reset: { scene:'campaign reset retreat', challenge:'admit mistakes and reset tone', audience:'donors and staff' },
  rebrand_tour: { scene:'image reboot tour launch', challenge:'show change without seeming opportunistic', audience:'skeptical voters' },
  hail_mary_msg: { scene:'final-week hail-mary message', challenge:'swing the race in one move', audience:'late deciders' },
};
...
function _getActionDefById(actionId){
  return ACTIONS.find(a=>a.id===actionId) || UNDERDOG_ACTIONS.find(a=>a.id===actionId) || null;
}

function _buildActionSpecificScenario(actionId){
  const a = _getActionDefById(actionId);
  const hint = AI_MG_ACTION_HINTS[actionId];
  if(!a || !hint) return null;
  return {
    situation: `${hint.scene} — ${a.name}`,
    prompts: [
      `In this ${hint.scene}, give your best answer to ${hint.audience} right now.`,
      `A challenger pushes back on ${a.name}. Defend your plan in one tight answer.`,
      `Close by addressing how you will ${hint.challenge} this week.`,
    ],
  };
}

function renderAIMinigame(el, actionId){
  const actionDef = _getActionDefById(actionId);
  const specificScenario = _buildActionSpecificScenario(actionId);
  const scenPool = AI_MG_SCENARIOS[actionId] || (specificScenario ? [specificScenario] : [AI_MG_GENERIC]);
  const meta     = AI_MG_META[actionId] || {
    icon: actionDef?.icon || '🗣',
    label: (actionDef?.name || 'CAMPAIGN EVENT').toUpperCase(),
    color:'#c8a84b'
  };
...
function openMinigame(actionId){
  ...
  modal.classList.add('active');
  // Close btn stays visible as a cancel escape hatch (clicking it skips the minigame at 0 bonus)
}
...
// Close button is always visible — this is kept for backward compat with inline CTA buttons
function showMGCloseBtn(){ /* close btn is always visible now — no-op */ }
...
function showConvention(){
  ...
  html+=`</div>`;
  if(!won){
    const floor = resolveContestedConvention();
    html += `<div style="background:rgba(255,255,255,.03);border:1px solid #1e2535;border-radius:8px;padding:12px;max-width:560px;font-size:12px;color:var(--text2);line-height:1.6;margin-top:4px">
      <div style="font-family:var(--font-mono);font-size:9px;color:var(--accent);letter-spacing:.1em;margin-bottom:6px">CONTESTED CONVENTION FLOOR FIGHT</div>
      ${floor.rounds.map((r,idx)=>`<div style="margin-bottom:5px"><strong style="color:${idx===floor.rounds.length-1?'var(--accent)':'#c8d0e0'}">Ballot ${idx+1}:</strong> ${r}</div>`).join('')}
    </div>`;
    if(floor.won){
      GS.playerDelegates=needed;
      html+=`<div style="background:rgba(200,168,75,.1);border:1px solid var(--accent);border-radius:8px;padding:14px;max-width:520px;font-size:13px;color:var(--text2);margin-top:10px">- ${floor.summary} <strong style="color:var(--accent)">You win the nomination!</strong></div>`;
    } else {
      html+=`<div style="background:rgba(239,68,68,.1);border:1px solid var(--rep);border-radius:8px;padding:14px;max-width:520px;font-size:13px;color:var(--text2);margin-top:10px">-- ${floor.summary} <strong style="color:var(--rep)">Your campaign ends here.</strong></div><button class="start-btn" style="margin-top:14px" onclick="showEndScreen(false,'convention')">View Results</button>`;
...
function buildPrimaryElectionNightCalls(){
  const regional = GS._aiRegional || {};
  const activeAIs = GS.aiCandidates.filter(a=>a.active);
  const calls = GS.states.map(state=>{
    const playerScore = Math.max(0, GS.favorability + state.primLead + (GS.groundGame-50)*0.12);
    const aiScores = activeAIs.map(a=>{
      const rb = ((regional[state.code] && regional[state.code][a.id]) || 0) * (a.approval/45);
      return { id:a.id, name:a.name, score: Math.max(0, a.approval + rb) };
    });
    const bestAI = aiScores.sort((a,b)=>b.score-a.score)[0] || { id:'ai', name:'Opponent', score:0 };
    const winner = playerScore >= bestAI.score ? 'player' : bestAI.id;
    const margin = Math.abs(playerScore - bestAI.score);
    return {
      code: state.code,
      name: state.name,
      delegates: state.pd,
      winner,
      winnerName: winner==='player' ? GS.playerName : bestAI.name,
      margin
    };
  });

  // Safe states first, nail-biters later for TV drama.
  return calls.sort((a,b)=>b.margin-a.margin);
}

function showPrimaryElectionNight(){
  if(GS._primaryNightDone){ showConvention(); return; }
  GS.phaseTransitioning = true;
  GS._primaryNightInProgress = true;
  showScreen('convention-screen');
  const el = document.getElementById('convention-content');
  if(!el) return;

  const calls = buildPrimaryElectionNightCalls();
  let idx = 0;
  let playerProjected = 0;
  const rivalProjected = {};
  let feed = '';

  el.innerHTML = `
    <div class="convention-title animate-in">📺 Primary Election Night</div>
    <div class="convention-msg animate-in">Week 14 is complete. Polls are closing state-by-state across the country.</div>
    <div style="width:min(760px,95%);display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px">
      <div style="background:#0d1117;border:1px solid #1e2535;border-radius:8px;padding:12px">
        <div style="font-family:var(--font-mono);font-size:9px;color:var(--accent);letter-spacing:.12em;margin-bottom:8px">PROJECTED DELEGATE BOARD</div>
        <div id="primary-night-board" style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:var(--text2)"></div>
      </div>
      <div style="background:#0d1117;border:1px solid #1e2535;border-radius:8px;padding:12px">
        <div style="font-family:var(--font-mono);font-size:9px;color:#60a5fa;letter-spacing:.12em;margin-bottom:8px">LIVE CALL FEED</div>
        <div id="primary-night-feed" style="max-height:260px;overflow:auto;font-size:12px;line-height:1.55;color:#c8d0e0"></div>
      </div>
    </div>
    <div id="primary-night-footer" style="margin-top:12px;font-size:12px;color:var(--text3)"></div>
  `;

  function renderBoard(){
    const board = document.getElementById('primary-night-board');
    if(!board) return;
    const rivals = GS.aiCandidates.map(a=>{
      const d = rivalProjected[a.id] || 0;
      return `<div style="display:flex;justify-content:space-between"><span>${a.name}</span><b style="color:${a.color||'#94a3b8'}">${d.toLocaleString()}</b></div>`;
    }).join('');
    board.innerHTML = `
      <div style="display:flex;justify-content:space-between;border-bottom:1px solid #1e2535;padding-bottom:6px;margin-bottom:4px">
        <span>${GS.playerName}</span><b style="color:var(--accent)">${playerProjected.toLocaleString()}</b>
      </div>
      ${rivals}
    `;
  }

  function addCallLine(line){
    const feedEl = document.getElementById('primary-night-feed');
    if(!feedEl) return;
    feed += `<div style="margin-bottom:6px">${line}</div>`;
  ...
  const timer = setInterval(()=>{
    if(idx >= calls.length){
      clearInterval(timer);
      GS._primaryNightDone = true;
      GS._primaryNightInProgress = false;
      const topRival = GS.aiCandidates.slice().sort((a,b)=>(rivalProjected[b.id]||0)-(rivalProjected[a.id]||0))[0];
      const lead = playerProjected - (topRival ? (rivalProjected[topRival.id]||0) : 0);
      const footer = document.getElementById('primary-night-footer');
      if(footer){
        footer.innerHTML = `Final projection: <b style="color:var(--accent)">${GS.playerName} ${playerProjected.toLocaleString()}</b> delegates · ` +
          `<span style="color:#94a3b8">Lead ${lead>=0?'+':''}${lead.toLocaleString()}</span>`;
      }
      const proceedBtn = `<button class="start-btn" style="margin-top:14px" onclick="finishPrimaryElectionNight()">Proceed to Convention →</button>`;
      if(footer) footer.innerHTML += `<div>${proceedBtn}</div>`;
      return;
    }

    const c = calls[idx++];
    if(c.winner === 'player') playerProjected += c.delegates;
    else rivalProjected[c.winner] = (rivalProjected[c.winner]||0) + c.delegates;
    renderBoard();
    const color = c.winner === 'player' ? 'var(--accent)' : '#60a5fa';
    addCallLine(`<span style="color:#4a5568">${c.code}</span> called for <b style="color:${color}">${c.winnerName}</b> · ${c.delegates} delegates`);
  }, 120);
}

function finishPrimaryElectionNight(){
  GS.phaseTransitioning = false;
  showConvention();
}

function resolveContestedConvention(){
  const rivals=[...GS.aiCandidates].sort((a,b)=>b.delegates-a.delegates);
  const topRival=rivals[0]||{name:'Top Rival',delegates:0};
  const total=GS.playerDelegates+rivals.reduce((s,a)=>s+a.delegates,0);
  const playerShare=GS.playerDelegates/Math.max(total,1);
  const leadMargin=GS.playerDelegates-topRival.delegates;

  // Protect players who clearly won the map but missed strict majority.
  if(playerShare>=0.42&&leadMargin>=260){
    return{
      won:true,
      rounds:[
        `You entered with a dominant plurality (+${leadMargin.toLocaleString()} over ${topRival.name}).`,
        `Party leaders quickly consolidated uncommitted delegates behind your ticket.`,
        `By the second ballot the race was effectively over.`
      ],
      summary:`Your delegate advantage was too large for rivals to overturn in a contested primary.`
    };
  }

  let playerScore = GS.playerDelegates + GS.endorsements*55 + Math.max(0,GS.momentum)*18 + Math.max(0,GS.favorability-45)*8;
  let rivalScore  = topRival.delegates + Math.max(0,50-GS.favorability)*6 + Math.max(0,-GS.momentum)*14;
  const rounds=[];

  for(let ballot=1; ballot<=3; ballot++){
    const persuasion = 28 + ballot*10;
    const playerGain = Math.round(cl((playerScore-rivalScore)/130 + G(0,22), -12, 34) + persuasion*0.2);
    const rivalGain  = Math.round(cl((rivalScore-playerScore)/140 + G(0,20), -10, 30) + persuasion*0.15);
    playerScore += playerGain;
    rivalScore += rivalGain;
    rounds.push(`Whip teams trade commitments: your bloc gains ${Math.max(0,playerGain)} influence points while ${topRival.name} gains ${Math.max(0,rivalGain)}.`);
  }

  const finalLead=playerScore-rivalScore;
  const won = finalLead>=0 || (playerShare>=0.36 && finalLead>-35);
  const summary = won
    ? `After three ballots and backroom negotiations, you edged ${topRival.name} by ${Math.max(1,Math.round(Math.abs(finalLead)))} convention points.`
    : `${topRival.name} assembled a late coalition and beat your floor operation by ${Math.max(1,Math.round(Math.abs(finalLead)))} convention points.`;

  return{won,rounds,summary};
}
...
function closeSummary(){
  ...
  if(GS.phase==='primary'&&GS.week>GS.primaryWeeks&&!GS.phaseTransitioning){
    GS.phaseTransitioning=true;
    setTimeout(()=>{showPrimaryElectionNight();},400);
    return;
  }
